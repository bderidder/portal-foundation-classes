...
public abstract class ValidationRule implements IValidatable
{
			...
			String description)
	{
		...
		{
			validationContext.validationFailed(description);
		}
	}
	...
	protected final void validationPassed(IValidationContext validationContext)
	{
		if (validationContext != null)
		{
			validationContext.validationPassed();
		}
	}

	public final void validate(IValidationContext validationContext)
	{
		if (validateRule(validationContext))
		{
			validationPassed(validationContext);
		}
		else
		{
			validationFailed(validationContext, null);
		}
	}
