...
public class PasswordInput extends FormElement<IPropertyModel<String>>
{
    ...
    public PasswordInput(IPropertyModel<String> model)
    {
        super(model);
    }
    ...
    public void draw(IRenderContext pRenderContext) throws RenderException
    {
        ...
        addAttribute("type", "password", buffer);
        ...
        if (!isValid())
        {
            ...
            //			addAttribute("title", getTextInputModel().getErrorDescription(),
            //					buffer);
        }
        else
        {
